 ...
 */
class DebugContextForTestsTest extends TestCaseBase
{
    /**
    ...
    {
        $actual = DebugContextForTests::getContextsStack();
        ...
        foreach (IterableUtil::zip(array_keys($expected), array_keys($actual)) as [$expectedCtxIndex, $actualCtxDesc]) {
            ...
            foreach (IterableUtil::zip(array_keys($expectedCtx), array_keys($actualCtx)) as [$expectedKey, $actualKey]) {
    ...
    {
        $actualContextsStack = DebugContextForTests::getContextsStack();
    ...
    {
        DebugContextForTests::newScope(/* out */ $dbgCtx, ['my_key' => 1]);
    ...
    {
        DebugContextForTests::newScope(/* out */ $dbgCtx, ['my context' => 'before func']);
        ...
        $secondFunc =
             ...
             * @param Pair<string, array<string, mixed>>[] $expectedContextsStackFromCaller
             */
            static function (array $expectedContextsStackFromCaller): void {
                /**
                 * @var Pair<string, array<string, mixed>>[] $expectedContextsStackFromCaller
                 */
                DebugContextForTests::newScope(/* out */ $dbgCtx, ['my context' => 'func entry']);
    ...
    {
        DebugContextForTests::newScope(/* out */ $dbgCtx, ['my context' => 'before calling 2nd func']);
        ...
        $secondFunc = static function (array $expectedContextsStackFromCaller) use ($SubScopeShouldExitEarly): void {
             ...
             * @var Pair<string, array<string, mixed>>[] $expectedContextsStackFromCaller
             */
            DebugContextForTests::newScope(/* out */ $dbgCtx, ['my context' => 'before sub-scope']);
    ...
    {
        DebugContextForTests::newScope(/* out */ $dbgCtx, ['my context' => 'inside recursive func before recursive call']);
        ...
        $depth = $currentDepth;
        foreach (DebugContextForTests::getContextsStack() as $actualCtxDesc => $actualCtx) {
    ...
    {
        DebugContextForTests::newScope(/* out */ $dbgCtx, ['my context' => 'before recursive func']);
    ...
    {
        return DebugContextForTests::funcArgs();
    }
    ...
    public function testFuncArgs(int $intParam, ?string $nullableStringParam, array $expectedArgs): void
    {
        ...
        foreach (IterableUtil::zip(array_keys($expectedArgs), array_keys($actualArgs)) as [$expectedParamName, $actualParamName]) {
    ...
    public function testCaptureVarByRef(): void
    {
        ...
        $capturedCtxStack = DebugContextForTests::getContextsStack();
