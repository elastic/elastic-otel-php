...
main() {
    ...
    if [ "$SKIP_NOTICE" = true ]; then
        ...
        GEN_NOTICE="\
            ...
            && php ./packaging/notice_generator.php >> /docker_host_dst/NOTICE \
        "
    fi
    ...
    for PHP_version_no_dot in "${PHP_versions_no_dot[@]}"; do
        ...
        local vendor_prod_dir="${work_repo_root_dir}/prod/php/vendor_${PHP_version_no_dot}"
        mkdir -p "${vendor_prod_dir}"
            # TODO: Sergey Kleyman: UNCOMMENT: scope_PHP_deps.sh
#        local vendor_prod_scoped_dir="${vendor_prod_dir}_scoped"
#        mkdir -p "${vendor_prod_scoped_dir}"
        ...
        docker run --rm \
            ...
            -v "${vendor_prod_dir}/:/docker_host_dst/vendor_prod/" \
            -v "${work_repo_root_dir}/NOTICE:/docker_host_dst/NOTICE" \
            ...
            sh -c "\
                ...
                && cp -r ./vendor_prod/. /docker_host_dst/vendor_prod/ \
...
#            -v "${vendor_prod_scoped_dir}/:/docker_host_dst/vendor_prod_scoped/" \

#                && ./tools/build/scope_PHP_deps.sh --input_dir ./vendor_prod --output_dir ./vendor_prod_scoped \
#                && cp -r ./vendor_prod_scoped/. /docker_host_dst/vendor_prod_scoped/ \
#                && chown -R ${current_user_id}:${current_user_group_id} /docker_host_dst/vendor_prod_scoped/ \
#                && chmod -R +r,u+w /docker_host_dst/vendor_prod_scoped/ \
#                \
