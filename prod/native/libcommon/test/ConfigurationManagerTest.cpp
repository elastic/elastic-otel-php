...
TEST_F(ConfigurationManagerTest, getConfigFromEnvVar_ElasticOptions) {
    EXPECT_CALL(iniMock_, getIniValue(::testing::_)).Times(::testing::AnyNumber()).WillRepeatedly(::testing::Return(std::nullopt));

    ConfigurationSnapshot snapshot;
    ASSERT_EQ(snapshot.revision, 0u);

    setenv("ELASTIC_OTEL_BOOTSTRAP_PHP_PART_FILE", "some_value", 1);

    cfg_.update();
    cfg_.updateIfChanged(snapshot);

    ASSERT_EQ(snapshot.ELASTIC_OTEL_BOOTSTRAP_PHP_PART_FILE, "some_value");

    setenv("ELASTIC_OTEL_BOOTSTRAP_PHP_PART_FILE", "some other value", 1);
    cfg_.update();
    cfg_.updateIfChanged(snapshot);
    ASSERT_EQ(snapshot.ELASTIC_OTEL_BOOTSTRAP_PHP_PART_FILE, "some other value");
}
